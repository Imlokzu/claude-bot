...
# stored value only.
CATALOG: tuple[Spec, ...] = (
    Spec("agents.defaults.thinkingDefault", "agent", "enum",
         ("off", "minimal", "low", "medium", "high", "xhigh", "adaptive", "max", "ultra")),
    Spec("agents.defaults.timeoutSeconds", "agent", "int", minimum=0, maximum=3600),
    Spec("agents.defaults.userTimezone", "agent", "string", max_length=64),
    Spec("agents.defaults.maxConcurrent", "agent", "int", minimum=1, maximum=32),
    Spec("agents.defaults.imageQuality", "agent", "enum",
         ("auto", "efficient", "balanced", "high"), default="auto"),
    Spec("agents.defaults.compaction.enabled", "context", "bool", default=True),
    Spec("agents.defaults.compaction.notifyUser", "context", "bool", default=False),
    Spec("agents.defaults.compaction.recentTurnsPreserve", "context", "int",
         default=3, minimum=0, maximum=20),
    Spec("agents.defaults.compaction.memoryFlush.enabled", "context", "bool", default=True),
    Spec("agents.defaults.startupContext.enabled", "context", "bool", default=True),
    Spec("tools.web.search.enabled", "web", "bool", default=True),
    Spec("tools.web.search.maxResults", "web", "int", default=5, minimum=1, maximum=10),
    Spec("tools.web.fetch.enabled", "web", "bool", default=True),
    Spec("tools.links.enabled", "web", "bool", default=True),
    Spec("tools.media.image.enabled", "media", "bool", default=True),
    Spec("tools.media.audio.enabled", "media", "bool", default=True),
    Spec("tools.media.video.enabled", "media", "bool", default=True),
    Spec("tools.media.audio.echoTranscript", "media", "bool", default=False),
    Spec("session.dmScope", "session", "enum",
         ("main", "per-peer", "per-channel-peer", "per-account-channel-peer"), default="main"),
    Spec("session.reset.idleMinutes", "session", "int", minimum=0, maximum=10080),
    Spec("tools.sessions.visibility", "session", "enum",
         ("self", "tree", "agent", "all"), default="all"),
    Spec("messages.inbound.debounceMs", "messages", "int", default=0, minimum=0, maximum=10000),
    Spec("messages.ackReactionScope", "messages", "enum",
         ("group-mentions", "group-all", "direct", "all", "off", "none")),
    Spec("messages.groupChat.historyLimit", "messages", "int", minimum=0, maximum=200),
    Spec("tools.exec.mode", "exec", "enum",
         ("deny", "allowlist", "ask", "auto", "full")),
    Spec("tools.exec.security", "exec", "enum", ("deny", "allowlist", "full")),
    Spec("tools.exec.ask", "exec", "enum", ("off", "on-miss", "always")),
    Spec("tools.fs.workspaceOnly", "exec", "bool", default=False),
    Spec("tools.elevated.enabled", "exec", "bool", default=False),
    Spec("memory.search.enabled", "memory", "bool", default=True),
    Spec("memory.search.query.maxResults", "memory", "int", default=6, minimum=1, maximum=20),
    Spec("browser.enabled", "browser", "bool", default=True),
    Spec("browser.headless", "browser", "bool", default=True),
    Spec("tts.enabled", "tts", "bool", default=False),
    Spec("tts.auto", "tts", "enum", ("off", "always", "inbound", "tagged"), default="off"),
    Spec("commands.text", "commands", "bool", default=True),
    Spec("commands.restart", "commands", "bool", default=True),
    Spec("commands.config", "commands", "bool", default=False),
)
