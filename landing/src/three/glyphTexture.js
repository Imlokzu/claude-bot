...
const CUSTOM_ICONS = {
  ...
  grok: {
    viewBox: 163.53,
    // the xAI/Grok app-icon slash mark (cropped from their official lockup,
    // not the "Grok" wordmark and not a generic X)
    path: "M105.02 34.51 38.72 129.19 58.68 129.19 124.98 34.51 105.02 34.51 Z",
    frame: true, // draw a rounded-square outline so it reads as a self-contained app icon
  },
  glm: {
    viewBox: 30,
...
export function createGlyphTexture({ id, icon, accent }) {
  ...
  if (frame) {
    // a rounded-square outline so single-glyph marks (Grok's slash, GLM's Z)
    // still read as a self-contained app icon, not a stray line
    ctx.save();
    ctx.shadowBlur = 14;
