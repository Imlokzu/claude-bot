...
export interface PromptBarModel {
  key: string;
  /** Рядком або розміткою: компонент вставляє назву як є і в згорнутий
      рядок, і в список, тож значки моделі можна класти просто сюди.
      Пошуку по назві для моделей у компоненті немає — на рядок він
      покладається лише для `sources` і `commands`. */
  name: ReactNode;
  /** Так само вставляється як є — у нас це роль моделі в ланцюжку. */
  tag?: ReactNode;
}
