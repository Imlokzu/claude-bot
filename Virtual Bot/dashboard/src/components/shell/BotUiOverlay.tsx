...
export function BotUiOverlay() {
  ...
  return (
    <aside className="pointer-events-none fixed inset-x-3 bottom-20 z-[var(--z-modal)] flex justify-end sm:inset-x-auto sm:right-[260px] sm:w-[min(390px,calc(100vw-24px))]" aria-live="polite">
      <section className="pointer-events-auto w-full rounded-lg border border-accent/35 bg-surface p-3 shadow-pop u-pop">
        ...
        {card.kind === 'question' || card.kind === 'choice' ? (
          <div className="grid gap-1.5">
            {options.map((option) => (
              <button key={option.label} type="button" onClick={() => { send(option.label); setCard(null); }} className="flex items-center gap-2 rounded-md border border-line px-2.5 py-2 text-left text-[13px] text-ink-2 transition-colors hover:border-accent hover:bg-accent-soft hover:text-ink">
            ...
            {card.kind === 'question' && data.allow_custom !== false ? (
              <form className="mt-1 flex gap-1.5" onSubmit={(event) => { event.preventDefault(); send(custom); if (custom.trim()) setCard(null); }}>
                ...
                <button type="submit" aria-label={t('question.send')} className="rounded-md bg-accent px-2.5 text-accent-ink"><Send size={14} /></button>
              </form>
            ) : null}
          </div>
        ) : card.kind === 'todo' ? (
